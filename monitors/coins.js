...
module.exports = class {
  ...
  static givePoints(client, message, level) { // eslint-disable-line no-unused-vars
    ...
    const score = client.coins.get(`${message.guild.id}-${message.author.id}`) || { coins: 0, level: 0, user: message.author.id, guild: message.guild.id, daily: 1504120109 };
    ...
    setTimeout(() => {
      ...
      score.coins += points;
      console.log(`Awarded ${points} coins to ${message.author.username} (${message.author.id}).`);
      ...
      client.coins.set(`${message.guild.id}-${message.author.id}`, score);
